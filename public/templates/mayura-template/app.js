document.addEventListener('DOMContentLoaded', () => {
  ...
  function applyDynamicMonogramAndNames(groom, bride) {
    ...
    document.querySelectorAll('.monogram, .myr-nav-mark, .myr-monogram, .nav-mark, .couple-mark, #couple-mark, .mark-tag, .logo-monogram, [data-bind="mark"]').forEach(el => {
      el.textContent = monogramDot;
    });
    document.querySelectorAll('a.myr-logo, .myr-brand-mark').forEach(el => {
      el.textContent = monogramDot;
    });
  }
  ...
  const navToggle = document.querySelector('.myr-nav-toggle, #myr-nav-toggle, .rjm-nav-toggle');
  const navLinks = document.querySelector('#myr-menu, .myr-nav-links, #rjm-menu');
