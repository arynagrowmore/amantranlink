...
async function main() {
  const targetUrl = 'https://www.jointhejashn.com/demo/mayura';
  console.log('Fetching Mayura:', targetUrl);
  ...
  // Find all image / asset URLs in HTML & CSS
  const assetRegex = /(?:src|href|content)=["']([^"']+\.(?:webp|png|jpg|jpeg|svg|gif|woff2|woff|ttf|mp3|mp4|webm|ico)[^"']*)["']/gi;
  ...
  // Search for any themes/mayura paths in HTML
  const themePaths = [...html.matchAll(/\/themes\/mayura\/[^"'\s\)]+/g)].map(m => m[0]);
  themePaths.forEach(p => assets.add('https://www.jointhejashn.com' + p));
  ...
  // Download JS scripts to analyze animations
  ...
  while ((match = scriptRegex.exec(html)) !== null) {
    ...
    try {
      ...
      if (jsCode.includes('myr-') || jsCode.includes('mayura') || jsCode.includes('Mayura')) {
        console.log('Found Mayura animation code in script:', filename);
        fs.writeFileSync(path.join(__dirname, 'mayura_app_chunk.js'), jsCode);
      }
    } catch (e) {}
  }

  console.log('Mayura theme extraction complete!');
}
