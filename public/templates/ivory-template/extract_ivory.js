...
async function main() {
  const targetUrl = 'https://www.jointhejashn.com/demo/ivory';
  console.log('Fetching Ivory:', targetUrl);
  ...
  // Search for any themes/ivory paths in HTML
  const themePaths = [...html.matchAll(/\/themes\/ivory\/[^"'\s\)]+/g)].map(m => m[0]);
  ...
  while ((match = scriptRegex.exec(html)) !== null) {
    ...
    try {
      ...
      if (jsCode.includes('ivy-') || jsCode.includes('ivory') || jsCode.includes('Ivory')) {
        console.log('Found Ivory animation code in script:', filename);
        fs.writeFileSync(path.join(__dirname, 'ivory_app_chunk.js'), jsCode);
      }
    } catch (e) {}
  }

  console.log('Ivory theme extraction complete!');
}
