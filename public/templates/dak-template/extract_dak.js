...
async function main() {
  const targetUrl = 'https://www.jointhejashn.com/demo/dak';
  console.log('Fetching Dâk:', targetUrl);
  ...
  // Search for any themes/dak paths in HTML
  const themePaths = [...html.matchAll(/\/themes\/dak\/[^"'\s\)]+/g)].map(m => m[0]);
  ...
  while ((match = scriptRegex.exec(html)) !== null) {
    ...
    try {
      ...
      if (jsCode.includes('dak-') || jsCode.includes('dak') || jsCode.includes('Dak') || jsCode.includes('dâk')) {
        console.log('Found Dâk animation code in script:', filename);
        fs.writeFileSync(path.join(__dirname, 'dak_app_chunk.js'), jsCode);
      }
    } catch (e) {}
  }

  console.log('Dâk theme extraction complete!');
}
