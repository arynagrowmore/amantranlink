...
async function main() {
  const targetUrl = 'https://www.jointhejashn.com/demo/jodi';
  console.log('Fetching Jodi:', targetUrl);
  ...
  // Search for any themes/jodi paths in HTML
  const themePaths = [...html.matchAll(/\/themes\/jodi\/[^"'\s\)]+/g)].map(m => m[0]);
  ...
  while ((match = scriptRegex.exec(html)) !== null) {
    ...
    try {
      ...
      if (jsCode.includes('jod-') || jsCode.includes('jodi') || jsCode.includes('Jodi')) {
        console.log('Found Jodi animation code in script:', filename);
        fs.writeFileSync(path.join(__dirname, 'jodi_app_chunk.js'), jsCode);
      }
    } catch (e) {}
  }

  console.log('Jodi theme extraction complete!');
}
