...
-- ==============================================================================
-- 📈 10. PARTNER ANALYTICS EVENTS TABLE (Funnel & Referral Intelligence)
-- ==============================================================================
CREATE TABLE IF NOT EXISTS public.partner_analytics_events (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  partner_id UUID REFERENCES public.profiles(id) ON DELETE CASCADE NOT NULL,
  event_type TEXT NOT NULL CHECK (event_type IN ('REFERRAL_VISIT', 'SIGNUP', 'INVITATION_CREATED', 'PREVIEW_SENT', 'CLIENT_APPROVED', 'PAYMENT_COMPLETED', 'PUBLISHED')),
  wedding_site_id UUID REFERENCES public.wedding_sites(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_partner_analytics_events_partner_type_created
  ON public.partner_analytics_events(partner_id, event_type, created_at);

-- ==============================================================================
-- 💳 11. PARTNER SETTLEMENTS TABLE (Internal Settlement Request & Accounting Ledger)
-- ==============================================================================
CREATE TABLE IF NOT EXISTS public.partner_settlements (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  partner_id UUID REFERENCES public.profiles(id) ON DELETE CASCADE NOT NULL,
  amount INTEGER NOT NULL CHECK (amount >= 500), -- Minimum settlement ₹500 (in INR or paise)
  status TEXT DEFAULT 'pending' CHECK (status IN ('pending', 'approved', 'processing', 'paid', 'rejected', 'reversed')),
...
-- Indexing for fast partner settlement queries
CREATE INDEX IF NOT EXISTS idx_partner_settlements_partner_status
  ON public.partner_settlements(partner_id, status, requested_at);

-- ==============================================================================
-- 📦 12. SUPABASE STORAGE BUCKET CONFIGURATION (Photos & Audio)
-- ==============================================================================
...
ALTER TABLE public.partner_analytics_events ENABLE ROW LEVEL SECURITY;
...
-- 📊 Partner Analytics Events RLS
DROP POLICY IF EXISTS "Partners can view own analytics events" ON public.partner_analytics_events;
CREATE POLICY "Partners can view own analytics events" ON public.partner_analytics_events FOR SELECT USING (auth.uid() = partner_id);

DROP POLICY IF EXISTS "Anyone can record analytics events" ON public.partner_analytics_events;
CREATE POLICY "Anyone can record analytics events" ON public.partner_analytics_events FOR INSERT WITH CHECK (true);

-- 💳 Partner Settlements RLS
DROP POLICY IF EXISTS "Partners can view own settlements" ON public.partner_settlements;
CREATE POLICY "Partners can view own settlements" ON public.partner_settlements FOR SELECT USING (auth.uid() = partner_id);

DROP POLICY IF EXISTS "Partners can request own settlements" ON public.partner_settlements;
CREATE POLICY "Partners can request own settlements" ON public.partner_settlements FOR INSERT WITH CHECK (auth.uid() = partner_id);

DROP POLICY IF EXISTS "Service role can manage settlements" ON public.partner_settlements;
CREATE POLICY "Service role can manage settlements" ON public.partner_settlements FOR ALL USING (true);
